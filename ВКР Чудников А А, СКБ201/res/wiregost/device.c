...
static int wg_vm_notification(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	{
		mutex_lock(&wg->device_update_lock);
		list_for_each_entry(peer, &wg->peer_list, peer_list) wg_noise_expire_current_peer_keypairs(peer);
		mutex_unlock(&wg->device_update_lock);
	}
	rtnl_unlock();
	return 0;
...
static void wg_setup(struct net_device *dev)
{
	...
	/*
	 * MESSAGE_MINIMUM_LENGTH comes from messages.h and should include
	 * Header + Auth Tag.
	 * For WireGost (Kuznyechik MGM), tag is 16 bytes, same as Poly1305.
	 * So the overhead calculation logic remains valid.
	 */
	const int overhead = MESSAGE_MINIMUM_LENGTH + sizeof(struct udphdr) + max(sizeof(struct ipv6hdr), sizeof(struct iphdr));
...
static int wg_newlink(struct net_device *dev, struct rtnl_newlink_params *params, struct netlink_ext_ack *extack)
{
	...
	wg->handshake_receive_wq = alloc_workqueue("wg-kex-%s", WQ_CPU_INTENSIVE | WQ_FREEZABLE | WQ_PERCPU, 0, dev->name);
	...
	wg->packet_crypt_wq = alloc_workqueue("wg-crypt-%s", WQ_CPU_INTENSIVE | WQ_MEM_RECLAIM | WQ_PERCPU, 0, dev->name);
...
static struct pernet_operations pernet_ops = {.pre_exit = wg_netns_pre_exit};
