...
static int send4(struct wg_device *wg, struct sk_buff *skb, struct endpoint *endpoint, u8 ds, struct dst_cache *cache)
{
	...
	if (!rt)
	{
		...
		if (IS_ERR(rt))
		{
			...
			pr_err("WireGost: send4 - Failed to resolve route to %pI4\n", &fl.daddr);
			goto err;
		}
		if (cache)
	...
	/* 13 Arguments for Kernel 6.x */
	udp_tunnel_xmit_skb(rt, sock, skb, fl.saddr, fl.daddr, ds, ip4_dst_hoplimit(&rt->dst), 0, fl.fl4_sport, fl.fl4_dport, false, true, /* nocheck */
						false /* 13th argument (always false for basic use) */
	);
	goto out;
...
static int send6(struct wg_device *wg, struct sk_buff *skb, struct endpoint *endpoint, u8 ds, struct dst_cache *cache)
{
#if IS_ENABLED(CONFIG_IPV6)
	struct flowi6	  fl  = {.saddr		   = endpoint->src6,
	...
	pr_info("WireGost: send6 - Sending %d bytes to %pI6 via %s\n", skb->len, &fl.daddr, dst->dev ? dst->dev->name : "NULL");

	udp_tunnel6_xmit_skb(dst, sock, skb, skb->dev, &fl.saddr, &fl.daddr, ds, ip6_dst_hoplimit(dst), 0, fl.fl6_sport, fl.fl6_dport, true, false);
	goto out;
...
{
	return (a->addr.sa_family == AF_INET && b->addr.sa_family == AF_INET && a->addr4.sin_port == b->addr4.sin_port &&
			a->addr4.sin_addr.s_addr == b->addr4.sin_addr.s_addr && a->src4.s_addr == b->src4.s_addr && a->src_if4 == b->src_if4) ||
		   (a->addr.sa_family == AF_INET6 && b->addr.sa_family == AF_INET6 && a->addr6.sin6_port == b->addr6.sin6_port &&
			ipv6_addr_equal(&a->addr6.sin6_addr, &b->addr6.sin6_addr) && a->addr6.sin6_scope_id == b->addr6.sin6_scope_id &&
...
int wg_socket_init(struct wg_device *wg, u16 port)
{
	struct net				  *net;
	int						   ret;
	struct udp_tunnel_sock_cfg cfg	= {.sk_user_data = wg, .encap_type = 1, .encap_rcv = wg_receive};
	struct socket			  *new4 = NULL, *new6 = NULL;
	struct udp_port_cfg		   port4 = {.family			   = AF_INET,
										.local_ip.s_addr   = htonl(INADDR_ANY),
										.local_udp_port	   = htons(port),
										.use_udp_checksums = false};
#if IS_ENABLED(CONFIG_IPV6)
	int					retries = 0;
...
retry:
	...
	/* Manually Disable Checksums */
	new4->sk->sk_no_check_tx = 1;
	new4->sk->sk_no_check_rx = 1;
	...
	if (ipv6_mod_enabled())
	{
		...
		new6->sk->sk_no_check_tx = 1;
		new6->sk->sk_no_check_rx = 1;
		set_sock_opts(new6);
