...
static inline void mod_peer_timer(struct wg_peer *peer, struct timer_list *timer, unsigned long expires)
{
	rcu_read_lock_bh();
...
{
	struct wg_peer *peer = timer_container_of(peer, timer, timer_retransmit_handshake);

	if (peer->timer_handshake_attempts > MAX_TIMER_HANDSHAKES)
	{
		pr_debug("%s: Handshake for peer %llu (%pISpfsc) did not complete after %d "
				 "attempts, giving up\n",
				 peer->device->dev->name, peer->internal_id, &peer->endpoint.addr, (int)MAX_TIMER_HANDSHAKES + 2);
	...
	else
	{
		...
		pr_debug("%s: Handshake for peer %llu (%pISpfsc) did not complete after %d "
				 ...
				 peer->device->dev->name, peer->internal_id, &peer->endpoint.addr, (int)REKEY_TIMEOUT, peer->timer_handshake_attempts + 1);
...
static void wg_expired_new_handshake(struct timer_list *timer)
{
	...
	pr_debug("%s: Retrying handshake with peer %llu (%pISpfsc) because we "
			 "stopped hearing back after %d seconds\n",
			 peer->device->dev->name, peer->internal_id, &peer->endpoint.addr, (int)(KEEPALIVE_TIMEOUT + REKEY_TIMEOUT));
...
static void wg_queued_expired_zero_key_material(struct work_struct *work)
{
	...
	pr_debug("%s: Zeroing out all keys for peer %llu (%pISpfsc), since we "
			 "haven't received a new one in %d seconds\n",
			 peer->device->dev->name, peer->internal_id, &peer->endpoint.addr, (int)REJECT_AFTER_TIME * 3);
...
{
	struct wg_peer *peer = timer_container_of(peer, timer, timer_persistent_keepalive);
...
void wg_timers_data_sent(struct wg_peer *peer)
{
	if (!timer_pending(&peer->timer_new_handshake))
		mod_peer_timer(peer, &peer->timer_new_handshake,
					   jiffies + (KEEPALIVE_TIMEOUT + REKEY_TIMEOUT) * HZ + get_random_u32_below(REKEY_TIMEOUT_JITTER_MAX_JIFFIES));
}
...
{
	mod_peer_timer(peer, &peer->timer_retransmit_handshake, jiffies + REKEY_TIMEOUT * HZ + get_random_u32_below(REKEY_TIMEOUT_JITTER_MAX_JIFFIES));
}
