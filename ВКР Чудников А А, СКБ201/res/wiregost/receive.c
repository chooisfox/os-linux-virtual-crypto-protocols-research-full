...
static int prepare_skb_header(struct sk_buff *skb, struct wg_device *wg)
{
	...
	{
		pr_err("WireGost: Invalid header len. Type=%d Len=%d\n", SKB_TYPE_LE32(skb), skb->len);
		return -EINVAL;
	}
...
static void wg_receive_handshake_packet(struct wg_device *wg, struct sk_buff *skb)
{
	...
	{
		pr_err("WireGost: Packet dropped due to MAC validation failure. State: %d\n", mac_state);
		return;
	}
	...
	switch (SKB_TYPE_LE32(skb))
	{
		case cpu_to_le32(MESSAGE_HANDSHAKE_INITIATION): {
			...
			if (unlikely(!peer))
			{
				pr_err("WireGost: Handshake Initiation Consume Failed (Decryption/Key Mismatch?)\n");
				return;
			}
		...
		case cpu_to_le32(MESSAGE_HANDSHAKE_RESPONSE): {
			...
			if (unlikely(!peer))
			{
				pr_err("WireGost: Handshake Response Consume Failed\n");
				return;
			}
			wg_socket_set_peer_endpoint_from_skb(peer, skb);
...
static void keep_key_fresh(struct wg_peer *peer)
{
	...
	send	= keypair && READ_ONCE(keypair->sending.is_valid) && keypair->i_am_the_initiator &&
		   wg_birthdate_has_expired(keypair->sending.birthdate, REJECT_AFTER_TIME - KEEPALIVE_TIMEOUT - REKEY_TIMEOUT);
	rcu_read_unlock_bh();
...
static bool decrypt_packet(struct sk_buff *skb, struct noise_keypair *keypair)
{
	...
	if (unlikely(!READ_ONCE(keypair->receiving.is_valid) || wg_birthdate_has_expired(keypair->receiving.birthdate, REJECT_AFTER_TIME) ||
	...
	header = (struct message_data *)skb->data;

	if (gost_kuznyechik_mgm_decrypt(&keypair->receiving.mgm_ctx, skb->data + sizeof(struct message_data), skb->data + sizeof(struct message_data),
									skb->len - sizeof(struct message_data), NULL, 0, nonce))
	{
		return false;
	}
...
static bool counter_validate(struct noise_replay_counter *counter, u64 their_counter)
{
	...
	if (unlikely(counter->counter >= REJECT_AFTER_MESSAGES + 1 || their_counter >= REJECT_AFTER_MESSAGES))
		goto out;
	++their_counter;
	...
	if (likely(their_counter > counter->counter))
	{
		...
		for (i = 1; i <= top; ++i)
			counter->backtrack[(i + index_current) & ((COUNTER_BITS_TOTAL / BITS_PER_LONG) - 1)] = 0;
...
static void wg_packet_consume_data_done(struct wg_peer *peer, struct sk_buff *skb, struct endpoint *endpoint)
{
	...
	if (unlikely(!(pskb_network_may_pull(skb, sizeof(struct iphdr)) &&
				   (ip_hdr(skb)->version == 4 || (ip_hdr(skb)->version == 6 && pskb_network_may_pull(skb, sizeof(struct ipv6hdr)))))))
...
int wg_packet_rx_poll(struct napi_struct *napi, int budget)
{
	...
	while ((skb = wg_prev_queue_peek(&peer->rx_queue)) != NULL && (state = atomic_read_acquire(&PACKET_CB(skb)->state)) != PACKET_STATE_UNCRYPTED)
	{
...
void wg_packet_decrypt_worker(struct work_struct *work)
{
	...
	while ((skb = ptr_ring_consume_bh(&queue->ring)) != NULL)
	{
		enum packet_state state = likely(decrypt_packet(skb, PACKET_CB(skb)->keypair)) ? PACKET_STATE_CRYPTED : PACKET_STATE_DEAD;
...
static void wg_packet_consume_data(struct wg_device *wg, struct sk_buff *skb)
{
	...
	rcu_read_lock_bh();
	PACKET_CB(skb)->keypair = (struct noise_keypair *)wg_index_hashtable_lookup(wg->index_hashtable, INDEX_HASHTABLE_KEYPAIR, idx, &peer);
	...
	ret = wg_queue_enqueue_per_device_and_peer(&wg->decrypt_queue, &peer->rx_queue, skb, wg->packet_crypt_wq);
...
void wg_packet_receive(struct wg_device *wg, struct sk_buff *skb)
{
	...
	{
		pr_err("WireGost: Header preparation failed\n");
		goto err;
	}
	...
	switch (SKB_TYPE_LE32(skb))
	{
		...
		case cpu_to_le32(MESSAGE_HANDSHAKE_COOKIE): {
			...
			queue_work_on(cpu, wg->handshake_receive_wq, &per_cpu_ptr(wg->handshake_queue.worker, cpu)->work);
			break;
		}
