...
# Parameters
# Increased file size to 512MB to get accurate timing on fast virtual links
FILE_SIZE_MB=512
TEST_DURATION=10
LOG_DIR="/tmp"
...
cleanup_common() {
    ...
    # Kill PIDs only if namespaces exist
...
# Trap interrupts to clean up if user presses Ctrl+C
...
setup_namespaces() {
    ...
    # Disable RP Filter for nested VPN testing
...
setup_vpn_impl() {
    # Default: Direct VETH acting as VPN for baseline testing
...
parse_iperf_json() {
    local file=$1
    local field=$2
    # Try Python for accurate JSON parsing (handles exponents/formatting better than grep)
    if command -v python3 &>/dev/null; then
        python3 -c "import sys, json; print(json.load(sys.stdin)['end']['sum_received']['$field'])" < "$file" 2>/dev/null
    else
        # Fallback to grep/awk (Less accurate but works without python)
...
run_udp_saturation_test() {
    ...
    # UDP Loss is often in a different JSON field
    if command -v python3 &>/dev/null; then
...
run_udp_pps_test() {
    ...
    # For PPS, we look at packet count / time
    if command -v python3 &>/dev/null; then
...
run_file_integrity_test() {
    ...
    if [ "$orig_sum" == "$recv_sum" ]; then
        ...
        # Prevent division by zero if too fast
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    # Explicit exit to prevent any loop artifacts
    exit 0
fi
