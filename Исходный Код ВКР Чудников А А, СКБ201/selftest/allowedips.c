...
static __init inline union nf_inet_addr horrible_cidr_to_mask(u8 cidr)
{
	...
	if (cidr % 32)
		mask.all[cidr / 32] = (__force u32)htonl((0xFFFFFFFFUL << (32 - (cidr % 32))) & 0xFFFFFFFFUL);
	return mask;
}

static __init inline u8 horrible_mask_to_cidr(union nf_inet_addr subnet)
{
	return hweight32(subnet.all[0]) + hweight32(subnet.all[1]) + hweight32(subnet.all[2]) + hweight32(subnet.all[3]);
}
...
{
	return (ip->in6_u.u6_addr32[0] & node->mask.ip6[0]) == node->ip.ip6[0] && (ip->in6_u.u6_addr32[1] & node->mask.ip6[1]) == node->ip.ip6[1] &&
		   (ip->in6_u.u6_addr32[2] & node->mask.ip6[2]) == node->ip.ip6[2] && (ip->in6_u.u6_addr32[3] & node->mask.ip6[3]) == node->ip.ip6[3];
}
...
static __init void horrible_insert_ordered(struct horrible_allowedips *table, struct horrible_allowedips_node *node)
{
	...
	{
		if (other->ip_version == node->ip_version && !memcmp(&other->mask, &node->mask, sizeof(union nf_inet_addr)) &&
...
static __init int horrible_allowedips_insert_v4(struct horrible_allowedips *table, struct in_addr *ip, u8 cidr, void *value)
{
...
static __init int horrible_allowedips_insert_v6(struct horrible_allowedips *table, struct in6_addr *ip, u8 cidr, void *value)
{
	struct horrible_allowedips_node *node = kzalloc(sizeof(*node), GFP_KERNEL);
...
static __init void horrible_allowedips_remove_by_value(struct horrible_allowedips *table, void *value)
{
	struct horrible_allowedips_node *node;
	struct hlist_node				*h;
...
static __init bool randomized_test(void)
{
	...
	for (i = 0; i < NUM_RAND_ROUTES; ++i)
	{
		...
		for (j = 0; j < NUM_MUTATED_ROUTES; ++j)
		{
			...
			for (; k < 4; ++k)
				mutate_mask[k] = 0;
			for (k = 0; k < 4; ++k)
				mutated[k] = (mutated[k] & mutate_mask[k]) | (~mutate_mask[k] & get_random_u8());
			cidr = get_random_u32_inclusive(1, 32);
			peer = peers[get_random_u32_below(NUM_PEERS)];
			if (wg_allowedips_insert_v4(&t, (struct in_addr *)mutated, cidr, peer, &mutex) < 0)
			{
				pr_err("allowedips random self-test malloc: FAIL\n");
				goto free_locked;
			}
	...
	for (i = 0; i < NUM_RAND_ROUTES; ++i)
	{
		...
		for (j = 0; j < NUM_MUTATED_ROUTES; ++j)
		{
			...
			cidr = get_random_u32_inclusive(1, 128);
			peer = peers[get_random_u32_below(NUM_PEERS)];
			if (wg_allowedips_insert_v6(&t, (struct in6_addr *)mutated, cidr, peer, &mutex) < 0)
			{
				pr_err("allowedips random self-test malloc: FAIL\n");
				goto free_locked;
			}
...
#define insert(version, mem, ipa, ipb, ipc, ipd, cidr) wg_allowedips_insert_v##version(&t, ip##version(ipa, ipb, ipc, ipd), cidr, mem, &mutex)

#define remove(version, mem, ipa, ipb, ipc, ipd, cidr) wg_allowedips_remove_v##version(&t, ip##version(ipa, ipb, ipc, ipd), cidr, mem, &mutex)
...
#define test_negative(version, mem, ipa, ipb, ipc, ipd)                                                                                            \
	do                                                                                                                                             \
	{                                                                                                                                              \
		bool _s = lookup(t.root##version, (version) == 4 ? 32 : 128, ip##version(ipa, ipb, ipc, ipd)) != (mem);                                    \
		maybe_fail();                                                                                                                              \
...
{
	bool			found_a = false, found_b = false, found_c = false, found_d = false, found_e = false, found_other = false;
	struct wg_peer *a = init_peer(), *b = init_peer(), *c = init_peer(), *d = init_peer(), *e = init_peer(), *f = init_peer(), *g = init_peer(),
				   *h = init_peer();
	...
	list_for_each_entry(iter_node, &a->allowedips_list, peer_list)
	{
		...
		if (cidr == 27 && family == AF_INET && !memcmp(ip, ip4(192, 95, 5, 64), sizeof(struct in_addr)))
			found_a = true;
		else if (cidr == 128 && family == AF_INET6 && !memcmp(ip, ip6(0x26075300, 0x60006b00, 0, 0xc05f0543), sizeof(struct in6_addr)))
			found_b = true;
		else if (cidr == 29 && family == AF_INET && !memcmp(ip, ip4(10, 1, 0, 16), sizeof(struct in_addr)))
			found_c = true;
		else if (cidr == 83 && family == AF_INET6 && !memcmp(ip, ip6(0x26075300, 0x6d8a6bf8, 0xdab1e000, 0), sizeof(struct in6_addr)))
			found_d = true;
		else if (cidr == 21 && family == AF_INET6 && !memcmp(ip, ip6(0x26075000, 0, 0, 0), sizeof(struct in6_addr)))
			found_e = true;
		else
			found_other = true;
	}
