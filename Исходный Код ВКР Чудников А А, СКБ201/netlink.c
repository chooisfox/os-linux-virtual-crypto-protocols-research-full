...
static int get_allowedips(struct sk_buff *skb, const u8 *ip, u8 cidr, int family)
{
	...
	if (nla_put_u8(skb, WGALLOWEDIP_A_CIDR_MASK, cidr) || nla_put_u16(skb, WGALLOWEDIP_A_FAMILY, family) ||
...
static int get_peer(struct wg_peer *peer, struct sk_buff *skb, struct dump_ctx *ctx)
{
	...
	if (!allowedips_node)
	{
		const struct __kernel_timespec last_handshake = {.tv_sec  = peer->walltime_last_handshake.tv_sec,
		...
		if (nla_put(skb, WGPEER_A_LAST_HANDSHAKE_TIME, sizeof(last_handshake), &last_handshake) ||
			...
			nla_put_u64_64bit(skb, WGPEER_A_RX_BYTES, peer->rx_bytes, WGPEER_A_UNSPEC) || nla_put_u32(skb, WGPEER_A_PROTOCOL_VERSION, 1))
			goto err;
...
int wg_get_device_dumpit(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	if (!ctx->next_peer)
	{
		if (nla_put_u16(skb, WGDEVICE_A_LISTEN_PORT, wg->incoming_port) || nla_put_u32(skb, WGDEVICE_A_FWMARK, wg->fwmark) ||
			nla_put_u32(skb, WGDEVICE_A_IFINDEX, wg->dev->ifindex) || nla_put_string(skb, WGDEVICE_A_IFNAME, wg->dev->name))
			goto out;

		down_read(&wg->static_identity.lock);
		if (wg->static_identity.has_identity)
		{
			if (nla_put(skb, WGDEVICE_A_PRIVATE_KEY, GOST_EC256_KEY_LEN, wg->static_identity.static_private) ||
	 ...
	 */
	if (list_empty(&wg->peer_list) || (ctx->next_peer && ctx->next_peer->is_dead))
	{
...
static int set_port(struct wg_device *wg, u16 port)
{
	...
	if (wg->incoming_port == port)
		return 0;
	list_for_each_entry(peer, &wg->peer_list, peer_list) wg_socket_clear_peer_endpoint_src(peer);
...
static int set_allowedip(struct wg_peer *peer, struct nlattr **attrs)
{
	...
	if (family == AF_INET && cidr <= 32 && nla_len(attrs[WGALLOWEDIP_A_IPADDR]) == sizeof(struct in_addr))
	{
		if (flags & WGALLOWEDIP_F_REMOVE_ME)
			ret = wg_allowedips_remove_v4(&peer->device->peer_allowedips, nla_data(attrs[WGALLOWEDIP_A_IPADDR]), cidr, peer,
										  &peer->device->device_update_lock);
		else
			ret = wg_allowedips_insert_v4(&peer->device->peer_allowedips, nla_data(attrs[WGALLOWEDIP_A_IPADDR]), cidr, peer,
										  &peer->device->device_update_lock);
	}
	else if (family == AF_INET6 && cidr <= 128 && nla_len(attrs[WGALLOWEDIP_A_IPADDR]) == sizeof(struct in6_addr))
	{
		if (flags & WGALLOWEDIP_F_REMOVE_ME)
			ret = wg_allowedips_remove_v6(&peer->device->peer_allowedips, nla_data(attrs[WGALLOWEDIP_A_IPADDR]), cidr, peer,
										  &peer->device->device_update_lock);
		else
			ret = wg_allowedips_insert_v6(&peer->device->peer_allowedips, nla_data(attrs[WGALLOWEDIP_A_IPADDR]), cidr, peer,
										  &peer->device->device_update_lock);
	}
...
static int set_peer(struct wg_device *wg, struct nlattr **attrs)
{
	...
	if (attrs[WGPEER_A_ALLOWEDIPS])
	{
		...
		{
			ret = nla_parse_nested(allowedip, WGALLOWEDIP_A_MAX, attr, NULL, NULL);
			if (ret < 0)
				goto out;
...
int wg_set_device_doit(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (info->attrs[WGDEVICE_A_PRIVATE_KEY] && nla_len(info->attrs[WGDEVICE_A_PRIVATE_KEY]) == GOST_EC256_KEY_LEN)
	{
		...
		if (!crypto_memneq(wg->static_identity.static_private, private_key, GOST_EC256_KEY_LEN))
			goto skip_set_private_key;

		/* We remove before setting, to prevent race, which means doing
		 * two genpub ops.
		 */
		if (gost_ec256_generate_public_key(public_key, private_key))
		{
...
static struct genl_family genl_family __ro_after_init = {.split_ops	  = wireguard_nl_ops,
