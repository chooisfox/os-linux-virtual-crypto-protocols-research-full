...
struct noise_symmetric_key {
	...
	u64 birthdate;
	bool is_valid;
	struct gost_kuznyechik_mgm_ctx mgm_ctx;
};
...
struct noise_static_identity {
	...
	u8 static_private[GOST_EC256_KEY_LEN];
...
struct noise_handshake {
	...
	u8 precomputed_static_static[NOISE_SYMMETRIC_KEY_LEN];
...
void wg_noise_handshake_init(struct noise_handshake *handshake, struct noise_static_identity *static_identity,
							 const u8 peer_public_key[NOISE_PUBLIC_KEY_LEN], const u8 peer_preshared_key[NOISE_SYMMETRIC_KEY_LEN],
							 struct wg_peer *peer);
void wg_noise_handshake_clear(struct noise_handshake *handshake);
static inline void wg_noise_reset_last_sent_handshake(atomic64_t *handshake_ns) {
	atomic64_set(handshake_ns, ktime_get_coarse_boottime_ns() - (u64)(REKEY_TIMEOUT + 1) * NSEC_PER_SEC);
}
